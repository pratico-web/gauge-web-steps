...
if (process.env.DEBUG == 'true') { console.log('Init called'); }
...
if (process.env.DEBUG == 'true') {  console.log('BEFORE Project Loader!!'); }
// loads the pageObjects and the tests steps
new GaugeWebStepsProjectLoader(siteMap).load(() => {
  gauge.siteMap = siteMap;
  gauge.expect = expect;
}).loadSteps();

if (process.env.DEBUG == 'true') {  console.log('AFTER Project Loader!!'); }
...
gauge.hooks.beforeScenario(function (context: any = null, done: any) {
  if (process.env.DEBUG == 'true') {  console.log('WdHelper.init!!!!'); }
  gauge.wdHelper.init(done);
});

gauge.hooks.afterScenario(function (context: any, done: any) {
  if (process.env.DEBUG == 'true') {  console.log('WdHelper.end!!!!'); }
