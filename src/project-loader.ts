...
import { GAUGE_DEFINITION_SCHEMA_FILE_NAME, GAUGE_PROJECT_DEFINITION_FILE_NAME, GAUGE_STEPS_FOLDER, GAUGE_PROJECT_DEFINITION_SCHEMA_NAME } from './models/constants';
...
export class GaugeWebStepsProjectLoader {
   ...
   */
  load(callback: Function): GaugeWebStepsProjectLoader {
  ...
  protected loadPageObjects() {
    if (process.env.DEBUG == 'true') {  console.log('Loading page Objects!!!!'); }
  ...
  loadSteps(): void {
    requireDir(path.join(this.rootFolder, GAUGE_STEPS_FOLDER));
  }
